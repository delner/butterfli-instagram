...
describe Butterfli::Instagram::Jobs::LocationRecentMedia do
  let(:options) { { } }
  let(:job) { Butterfli::Instagram::Jobs::LocationRecentMedia.new(options) }
  ...
  context "when comparing" do
    context "to a job with identical options" do
      ...
      let(:other_job) { Butterfli::Instagram::Jobs::LocationRecentMedia.new(options) }
      ...
      context "except obj_id" do
        let(:other_job) do
          Butterfli::Instagram::Jobs::LocationRecentMedia.new(options.merge(obj_id: obj_id + "1"))
        end
        it do
          expect(job).to_not eq(other_job)
      ...
      context "except min_id" do
        let(:other_job) do
          Butterfli::Instagram::Jobs::LocationRecentMedia.new(options.merge(min_id: min_id + "1"))
        end
        it do
          expect(job).to_not eq(other_job)
